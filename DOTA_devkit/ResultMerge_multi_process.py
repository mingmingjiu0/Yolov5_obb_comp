...
def mergesingle(dstpath, nms, fullname):
    ...
    #print('name:', name)
    dst_fullname = os.path.join(dstpath, name + '.txt')
    name_lists = name.split('_')
    dstname = os.path.join(dstpath, name_lists[0] + '_' + name_lists[1] + '.txt')
    with open(fullname, 'r') as f_in:
        ...
        for splitline in splitlines:
            ...
            poly = list(map(float, splitline[1:-1]))
            origpoly = poly2origpoly(poly, x, y, rate)
            det = origpoly
            det.append(splitline[0])
        ...
        with open(dstname, 'a') as f_out:
            for imgname in nameboxnmsdict:
                for det in nameboxnmsdict[imgname]:
                    ...
                    outline = str(int(label)) + ' ' + str(confidence) + ' ' + ' '.join(map(str, bbox))
...
def mergenms(dstpath, nms, fullname):
    dstname = os.path.join(dstpath, fullname.split('/')[-1])
    label_list = ['ship','car']
        ...
        with open(dstname, 'a') as f_out:
            for imgname in nameboxnmsdict:
                for det in nameboxnmsdict[imgname]:
                    ...
                    outline = ' '.join(map(str, bbox)) + ' ' + str(label_list[int(label)])
                    #print('outline:', outline)
                    f_out.write(outline + '\n')
...
def parse_args():
    ...
    parser.add_argument('--scrpath', default='/home/hzm/PycharmProjects/yolov5_obb/runs/detect/exp7/labels_merged', help='test config file path')
    parser.add_argument('--dstpath', default='/home/hzm/PycharmProjects/yolov5_obb/runs/detect/exp7/labels_merged_final', help='checkpoint file')
