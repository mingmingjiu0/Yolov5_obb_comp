...
class ComputeLoss:
    ...
    def build_targets(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            # Append
            torch_min = torch.tensor(0. ,device=targets.device)
            gj_ = torch.clamp(gj, torch_min, feature_wh[1]-1).long()
            gi_ = torch.clamp(gi, torch_min, feature_wh[0]-1).long()
            a = t[:, -1].long()  # anchor indices 取整
            # indices.append((b, a, gj.clamp_(0, gain[3] - 1), gi.clamp_(0, gain[2] - 1)))  # image, anchor, grid indices
            indices.append((b, a, gj_, gi_))  # image, anchor, grid indices
